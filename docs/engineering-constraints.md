...
- **Test timing:** write tests before the code they verify, at both granularities this pipeline uses. At the e2e level, `pipelinely-planning` commits the e2e specs (tagged `@pending`, expected to fail) before `pipelinely-dev` ever starts implementing, and `pipelinely-dev` must run them first and confirm they fail for the right reason before writing any implementation code. At the implementation level, follow `superpowers:test-driven-development`'s red-green-refactor loop for every function/behavior change: write the failing test, watch it fail, then write only the minimal code to pass — never write tests after the code already exists.
