---
name: pipelinely-planning
description: Kick off the planning stage of the cockpit pipeline — explores the repo, decides whether the work splits into milestones, names a real verifier, writes runnable e2e tests, and dispatches a fresh Opus 5 session to do that work. Invoke from the orchestrator's own session as `/pipelinely-planning <free-text instructions or a backlog item>`. Do NOT open a new tab for yourself — this runs in the orchestrator, the same way pipelinely does.
---
...
5. Write **real, runnable e2e tests** (the repo's existing e2e framework, e.g. Playwright) covering the plan's QA cases — not prose descriptions. `pipelinely-qa` executes these directly later.
...
Periodically (after `STATUS` changes, before `/pipelinely-handover`, or every ~15-20 tool calls) run `bash ~/Dev/pipelinely/scripts/write-metrics.sh` so the dashboard's CTX/MODEL/TOKENS/COST tiles stay live.
```
...
Once the fresh session finishes and `STATUS` reads `waiting: plan ready for review`, the developer triggers `/pipelinely-plan-review <slug>` themselves — this skill does not chain into it automatically.
