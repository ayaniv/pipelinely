---
name: pipelinely-qa-fixes
description: Fixes the QA failures the developer checked off in QA_TRIAGE.json, re-verifies, and hands off to a fresh pipelinely-qa re-run — continuing in this task's own existing dev tab rather than a fresh session, since fixing benefits from exactly the context a fresh QA reviewer deliberately doesn't have. Invoke directly inside the task's own tab as `/pipelinely-qa-fixes`.
---
...
2. Update `QA_REPORT.md` (rewrite it, not append) to reflect which cases are now resolved — keep the same `### Failing Cases (N)` / `### Passing Cases (N)` bullet format `pipelinely-qa` writes, **trailing location backtick-quoted** (`` - [Label] Description — `path/to/file.ts:42` ``), so the rewritten file parses exactly like a fresh QA pass would.
3. Append a `TIMELINE` line for stage `qa-fixes`.
4. Set `STATUS` to `waiting: fixes pushed, ready to re-run QA`.

## Step 5 — Hand back to QA

Don't grade your own fix by re-running the tests yourself and calling it done — that reintroduces exactly the "author is the worst judge" problem `pipelinely-qa`'s fresh-session design exists to avoid. Leave `STATUS` at `waiting: fixes pushed, ready to re-run QA` and let the developer re-trigger `/pipelinely-qa <slug>` for an independent re-check.
