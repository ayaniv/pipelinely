...
- Branch: claude/<slug>
- Branch status: new if the worktree didn't already exist (milestone child); existing if it did (flat task continuing from cockpit-planning)

## Mode: implement
Work test-driven. Do not mark STATUS done when finished — see Status reporting below.

## Context
<For a milestone: this milestone's name/description from the tech-design.md bullet, plus a pointer to the parent's tech-design.md for the wider project shape. For a flat task: its own tech-design.md if cockpit-planning wrote one.>
...
- Claim `ITERM_SESSION`/`TMUX_SESSION`: yes — this is the tab `cockpit-qa-fixes`/`cockpit-cr-fixes` will return to
