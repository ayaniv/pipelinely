...
CR runs before QA in this pipeline (Dev → CR → CR fixes → QA → QA fixes → Merge), so once the review comments are addressed the next automated step is a fresh `pipelinely-qa` pass against the fixed code — not a straight shot to manual merge. `pipelinely-cr` itself doesn't get re-triggered automatically; if the developer wants a second CR pass after a substantial fix, they can re-trigger `/pipelinely-cr <slug>` themselves before QA runs. Leave `STATUS` at `waiting: comments addressed, ready for QA` and let the developer (or the orchestrator) trigger `/pipelinely-qa <slug>` next.
